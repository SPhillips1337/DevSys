...
- Testing Agent: Runs configured tests (shell script or language-specific), writes test reports to artifacts and updates `task-store` (supports remote-ssh execution and report collection).
- Deployment Agent: Builds application images or runs `docker compose` stacks to deploy to a `dev` namespace; supports `host-docker` and `remote-ssh` (can optionally run `docker compose` remotely).
...
- Remote-ssh runner enforces `known_hosts` by default; fallback to insecure SSH is available via `REMOTE_ALLOW_INSECURE_SSH=true` for short-lived PoC only (not recommended).
...
- Phase 2 (PoC): Implement coding agent running opencode to scaffold a blog project, testing agent runs simple tests, deployment agent deploys to local compose webserver, monitoring agent does basic health checks. Remote-ssh runner, runner helper, sample manifest and tests, and a basic CI job have been implemented in PoC.
...
- `docker-compose.yml` with the services above and a `workspace` volume (examples updated to show secret mounts for SSH keys and known_hosts).
- `specs/` directory with a minimal task schema and one sample task: `specs/create-blog.yaml` and `specs/create-blog.json` (sample project manifest).
- `manager` service: HTTP API or CLI to accept user story and create tasks (now validates project manifests via JSON Schema and converts them to task specs).
- `coding-agent` image: scaffolds artifacts; PR flow skeleton exists (use `GITHUB_TOKEN` to enable GitHub operations).
- `testing-agent` runner: executes provided test script, supports remote-ssh execution, writes test reports and JUnit/XML results.
- `deployment-agent` deployer: supports local deploys and `remote-ssh` deploys; can optionally run `docker compose` remotely after transfer.
- `monitoring-agent` basic health checker and log scanner that can create tasks on failure.
- `utils/runner.py` helper for `auto` runner selection and secure remote copy/run utilities.
- CI: `.github/workflows/ci.yml` to run tests (added, with pip caching).
...
- Manager can accept a user story and create a validated task spec or accept a project manifest and convert it to a task spec.
- Coding Agent can pick up the task, run opencode, and produce source in `/workspace/tasks/<id>/src`.
- Testing Agent can run tests (locally or remotely) and report pass/fail back to the task-store.
- Deployment Agent can launch the site into the compose environment or copy artifacts to a remote host and optionally run `docker compose`.
...
- Finish spec-kit integration and add schema-based acceptance checks.
- Implement full GitHub PR flow in `coding-agent` using `GITHUB_TOKEN` and branch-per-task.
- Harden secrets: move to Docker secrets or integrate with an external secret manager.

If you approve, I'll begin by finishing spec-kit integration and implementing the GitHub PR flow.
